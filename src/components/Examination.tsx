...
const Examination = (props) => {
    ...
    useEffect(() => {
        database.getLatestPatientEventByType(patientId, EventTypes.ExaminationFull).then((response: any) => {
            if (response.length > 0) {
                const responseObj = JSON.parse(response)
                setExamination(responseObj.examination)
                setGeneralObservations(responseObj.generalObservations)
                setDiagnosis(responseObj.diagnosis)
                setTreatment(responseObj.treatment)
                setCovid19(responseObj.covid19)
